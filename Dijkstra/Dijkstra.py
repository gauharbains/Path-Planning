...
def coordinate_space(x_dim,y_dim):
    node=[]
...
def line_equation(coordinate_1,coordinate_2):
...
def semi_algebric_circle(centre_cd,radius):
    x=centre_cd[0]
    y=centre_cd[1]
    inside=[]    
...
def semi_algebric_ellipse(centre_cd,a,b):
    x=centre_cd[0]
    y=centre_cd[1]
    inside=[]    
...
################################################ Square Obstacle and Periphery Values
square=[]
for i in range(50,101):
    for j in range(68,113):
        square.append([i,j])
...
################################################################# Polygon Obstacle Space and Periphery Points
...
def dikjstra(start,goal,resolution):
    ...
    infinity=999999
    for node in node_id:
        open_set[node]=infinity
    open_set[unique_id(start)]=0
    ...
    while T:
        source_id= min(open_set, key=open_set.get)
        cost=open_set[source_id]
        ...
        for i, _ in enumerate(motion):
            ...
            if current==goal or (distance(current[0],current[1],goal[0],goal[1])<resolution):
            ...
            if (verify_node(current) and  (not(current_id in closed_set))):
                ...
                cost+=resolution*motion[i][2]       
                
                if open_set[current_id]>cost:
                    open_set[current_id]= cost
                    parent[current_id]=source_id
            cost=open_set[source_id]
...
path,nodes_created=dikjstra(start,goal,resolution)

for i in nodes_created:

#    print(i)
    
    cv.circle(img,(i[0],i[1]),2,[255,0,0]) 
    cv.imshow('img',np.flip(img,0))
    cv.waitKey(1)
#    cv.destroyAllWindows() 
cv.destroyAllWindows()  
for i in path:
    cv.circle(img,(i[0],i[1]),2,[0,255,0]) 
    cv.imshow('img',np.flip(img,0))
    cv.waitKey(1)
